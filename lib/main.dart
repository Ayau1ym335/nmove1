...
class DataService {
  /// Returns the current ESP32 base URL from SharedPreferences.
  /// The master ESP32 connects to the PHONE's hotspot (STA mode), so it gets
  /// a dynamic IP assigned by the phone — check Serial Monitor after boot.
  static Future<String> _espBaseUrl() async {
    final prefs = await SharedPreferences.getInstance();
    final saved = (prefs.getString('esp32_ip') ?? '').trim();
    return 'http://${saved.isEmpty ? '10.172.70.180' : saved}';
  }
...
static Future<List<IMUPacket>> fetchAndParseData() async {
  try {
    final baseUrl = await _espBaseUrl();
...
class _MainAppPlaceholderState extends State<MainAppPlaceholder> {
  static const bool _useEspDownload = true;

  final _esp32IpController = TextEditingController();
KneeSensorService sensorService = KneeSensorService();
...
double _toDouble(dynamic v) {
  ...
  Future<void> _loadSavedEsp32Ip() async {
    final prefs = await SharedPreferences.getInstance();
    final saved = (prefs.getString('esp32_ip') ?? '').trim();
    if (saved.isNotEmpty && mounted) {
      setState(() => _esp32IpController.text = saved);
    }
  }
...
Future<void> _syncViaWifi() async {
  final typedIp = _esp32IpController.text.trim();
  if (typedIp.isNotEmpty) await _saveEsp32Ip(typedIp);
  ...
  try {
    final prefs = await SharedPreferences.getInstance();
    final token = (prefs.getString('access_token') ?? '').trim();
    if (token.isEmpty) throw Exception('No access token. Please login first.');

    final gaitService = GaitSessionService(authToken: token);
    final esp32Ip = await GaitSessionService.getEsp32Ip();
    final espBaseUrl = 'http://$esp32Ip';

   // STEP 1 - Start recording
    setState(() => ageMessage = 'Step 1/5: Starting ESP32 recording...');
    try {
      final startResp = await http
          .get(Uri.parse('$espBaseUrl/start'))
          .timeout(const Duration(seconds: 8));
      if (startResp.statusCode != 200) {
        setState(() => ageMessage = 'ESP /start returned ${startResp.statusCode}');
      } else {
        setState(() => ageMessage = 'Recording... Walk for 10 seconds');
        // ← ДАЁМ ESP32 ВРЕМЯ ЗАПИСАТЬ ДАННЫЕ
        await Future.delayed(const Duration(seconds: 10));
      }
    } catch (e) {
      setState(() => ageMessage = 'Could not reach ESP32: $e');
    }

    // STEP 2
    setState(() => ageMessage = 'Step 2/5: Opening backend session...');
    final sessionId = await gaitService.startSessionOnBackend(
      legSide: 'left',
      deviceId: 'esp32',
    );
    if (sessionId == null || sessionId.isEmpty) {
      throw Exception('Failed to start backend session.');
    }

    // STEP 3 - Stop recording
    setState(() => ageMessage = 'Step 3/5: Stopping ESP32 recording...');
    try {
      await http
          .get(Uri.parse('$espBaseUrl/stop'))
          .timeout(const Duration(seconds: 8));
      // ← ДАЁМ ESP32 ВРЕМЯ ЗАКОНЧИТЬ ЗАПИСЬ В ФАЙЛ
      await Future.delayed(const Duration(seconds: 2));
    } catch (_) {}

    // STEP 4 - Download
    setState(() => ageMessage = 'Step 4/5: Downloading data from ESP32...');
    Uint8List? binData;
    try {
      final resp = await http
          .get(Uri.parse('$espBaseUrl/download'))
          .timeout(const Duration(seconds: 30));
      if (resp.statusCode == 200 && resp.bodyBytes.isNotEmpty) {
        binData = resp.bodyBytes;
        setState(() => ageMessage = 'Downloaded ${resp.bodyBytes.length} bytes!');
      } else {
        setState(() => ageMessage = 'ESP download failed (${resp.statusCode})');
      }
    } catch (e) {
      setState(() => ageMessage = 'ESP not reachable during download: $e');
    }
    // STEP 5
    setState(() => ageMessage = 'Step 5/5: Uploading to backend...');
if (binData == null) {
      setState(() => ageMessage = 'No ESP data. Session #$sessionId opened.');
      return;
    }

    final dir = await getTemporaryDirectory();
    final tempFile = File('${dir.path}/session_data.bin');
    await tempFile.writeAsBytes(binData);
...
if (!uploaded) throw Exception('Upload to backend failed.');

    // Сразу переходим на hero страницу
    setState(() {
      isDeviceConnected = true;
      ageMessage = 'Synced! Calculating...';
      selectedIndex = 0;
    });
    ...
    // Poll в фоне — данные подгрузятся сами
    ...
    if (sessionData != null && mounted) {
      ...
      final latest = snapshots.isNotEmpty && snapshots.last is Map<String, dynamic>
      ...
      if (mounted) {
        setState(() {
          ...
          if (movAge > 0) {
            legAge = movAge.round();
            final userAge = int.tryParse(
                UserDataProvider.of(context)?.userData.age ?? '25') ?? 25;
            final diff = (userAge - legAge).abs();
            if (legAge < userAge) {
              ageMessage = 'Your leg is $diff years younger than you!';
            } else if (legAge > userAge) {
              ageMessage = 'Your leg is $diff years older than you!';
            } else {
              ageMessage = 'Your leg age matches your real age!';
            }
    ...
    return SingleChildScrollView(
      ...
      child: Padding(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          const SizedBox(height: 12),
          // ── ESP32 IP input ──────────────────────────────────────────────────
          // The master ESP32 joins the PHONE's hotspot (STA mode) and receives
          // a dynamic IP from the phone's DHCP. Check Serial Monitor output:
          //   "IP адрес ESP32: 192.168.xxx.xxx"
          // Enter that IP here before pressing Download Data.
          Container(
            padding: const EdgeInsets.all(1.5),
            decoration: BoxDecoration(
            ...
            child: Container(
              ...
              child: TextField(
                ...
                decoration: const InputDecoration(
                  ...
                  hintText: 'ESP32 IP (e.g. 192.168.43.105)',
                  hintStyle: TextStyle(color: Colors.grey, fontSize: 14),
                  prefixIcon: Icon(Icons.router_outlined, color: Colors.cyanAccent, size: 20),
                  border: InputBorder.none,
                ),
                onChanged: (v) => _saveEsp32Ip(v),
              ),
            ),
          ),
          ...
          const Align(
            alignment: Alignment.centerLeft,
            child: Text(
              'Find ESP32 IP in Serial Monitor after booting with phone hotspot on.',
